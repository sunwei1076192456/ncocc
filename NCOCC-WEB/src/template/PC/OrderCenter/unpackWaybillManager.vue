...
<template>
  <div style="margin: 20px;">
    ...
    <!-- 新增运单 -->
    <Modal :mask-closable="false" :visible.sync="newModal" :loading="loading" v-model="newModal" width="750" title="新增拆箱单" @on-ok="newOk('wayBillNew')" @on-cancel="cancel()">
      <Form ref="wayBillNew" :model="wayBillNew" :rules="ruleNew" :label-width="80" class="newBillModal">
        ...
        <Row style="margin-bottom: 25px;">
          ...
          <Col span="6"><span style="color:red">*</span>发货人：
        ...
        <Row style="margin-bottom: 25px;">
          <Col span="6"><span style="color:red">*</span>收货点：
          <FormItem prop="handle_point">
            <Cascader :data="districtList" v-model="wayBillNew.handle_point" trigger="hover" style="width:150px"></Cascader>
          </FormItem>
          </Col>
          <Col span="6"><span style="color:red">*</span>收货状态：
          <FormItem prop="receive_goods_state">
            <Select v-model="wayBillNew.receive_goods_state" filterable clearable style="width: 150px">
              <Option v-for="item in ReceiveGoodStateList" :value="item.value" :key="item.value">{{ item.label }}</Option>
            </Select>
          </FormItem>
          </Col>
          <Col span="6"><span style="color:red">*</span>货盘码：
          <FormItem prop="good_pallet_code">
            <Input v-model="wayBillNew.good_pallet_code" style="width:150px"></Input>
          </FormItem>
          </Col>
        </Row>
        <Row style="margin-bottom: 25px;">
          ...
          <FormItem prop="unloading_goods_addr">
            <textarea v-model="wayBillNew.unloading_goods_addr" style="width:500px" :autosize="{minRows: 1,maxRows: 1}"></textarea>
          </FormItem>
          </Col>
        </Row>
        <Row style="margin-bottom: 25px;">
          ...
          <FormItem prop="contact_res">
            <Input v-model="wayBillNew.contact_res" style="width:150px"></Input>
          </FormItem>
          </Col>
          <Col span="6">收货联系电话：
        ...
        <Row style="margin-bottom: 25px;">
          ...
          <FormItem prop="productname">
            <Input v-model="wayBillNew.productname" style="width:150px"></Input>
          </FormItem>
          </Col>
          <Col span="6">货重：
          <FormItem prop="productweight">
            <Input v-model="wayBillNew.productweight" style="width:150px"></Input>
          </FormItem>
          </Col>
          <Col span="6">包装种类：
          ...
          <Col span="6"><span style="color:red">*</span>贸易类型：
    ...
    return {
      ...
      waybillcol: [{
        ...
        },
        {
          title: '收货状态',
          width: 100,
          key: 'receive_goods_state',
          align: 'center',
          render: (h, params) => {
            for (var i in this.ReceiveGoodStateList) {
              if (params.row.receive_goods_state == this.ReceiveGoodStateList[i].value) {
                if (params.row.receive_goods_state == 0) {
                  return h('div', [
                    h('strong', {
                      'style': {
                        'color': '#ed4014',
                    ...
                    }, this.ReceiveGoodStateList[i].label)
                  ]);
                } else if (params.row.receive_goods_state == 1) {
                  return h('div', [
                    h('strong', {
                      'style': {
                        'color': '#2db7f5',
                ...
                } else if (params.row.receive_goods_state == 2) {
                  return h('div', [
                    h('strong', {
                      'style': {
                        'color': '#ff9900',
      ...
      ruleNew: {
        customer_id: [
          { type: 'number', required: true, message: '请选择发货人', trigger: 'blur' }
        ],
        shipping_id: [
